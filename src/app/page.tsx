...
const homeHubReasons = [
  ...
  },
  {
    ...
    description:
      "홈이 서울·경기 전체 흐름과 예약 가이드를 담당하고, 지역 페이지가 영등포·강남·강서·송파처럼 실제 지역 의도를 받는 구조라면 같은 키워드를 무리하게 반복하지 않아도 됩니다. 이렇게 문서 목적을 분리하면 내부 경쟁을 줄이면서도 전체 사이트 신호를 더 강하게 만들 수 있습니다.",
  },
  {
...
const longformSections = [
  ...
  },
  {
    ...
    paragraphs: [
      "지역페이지는 영등포·강남·강서 출장마사지 대표 페이지처럼 실제 생활권 의도를 받는 문서입니다. 반면 홈은 서울·경기 전반의 큰 문맥과 이용 가이드를 담당하는 문서입니다. 이 둘을 섞어버리면 홈도 지역 신호가 흐려지고, 지역페이지도 상위 키워드와 충돌하게 됩니다.",
...
const additionalFaqs = [
  ...
  },
  {
    ...
    answer:
      "상위 홈 문서에서 서울·경기 전체 문맥을 설명하고, 하위 지역 문서에서 영등포·강남·강서·송파 같은 세부 생활권을 설명하는 계층 구조가 중요합니다. 이렇게 해야 지역 문맥이 자연스럽게 확장되고, 지역별 키워드도 서로 충돌하지 않고 쌓일 수 있습니다.",
  },
];
    ...
    <>
      ...
      <article>
        ...
        >
          ...
          <div className="prose">
            <p>
              홈이 출장마사지 같은 큰 키워드를 받는 상위 허브라면, 대표 지역 문서는 영등포·강남·강서·서울·경기처럼 실제 생활권 의도를 더
